...
\[
    \begin{aligned}
        \mathsf{c\_distance}&(\{B_C, B_{TTC}, B_{cs}, \mathsf{sns}, s_d, s_c, s_u, t\}) = \sum_{i=0}^{k} P(TTC_i \ge \mathsf{TH\_TTC\_c}) \geq \lceil k \cdot \mathsf{CONSENSUS}\rceil
    \end{aligned}
\]
...
\begin{definition}[Uncertain data]
    ...
    \[
        \mathsf{uncertain}(x) = \neg \mathsf{s\_distance}(x) \lor \neg \mathsf{s\_r\_distance}(x) \lor \neg \mathsf{r\_c\_distance}(x) \lor \neg \mathsf{c\_distance}(x)
    \]
\end{definition}
...
G(e_{23}) &= G(e_5) \\
G(e_{24}) &= G(e_6) \\
G(e_{25}) &= \{x \in X \mid (\neg \mathsf{valid\_d}(x) \lor \neg \mathsf{valid\_c}(x) \lor s_u \ge \mathsf{MAX\_UNCERTAIN}) \land \mathsf{sns} = 1 \land t = 0 \}\\
G(e_{26}) &= \{x \in X \mid (\neg \mathsf{valid\_d}(x) \lor \neg \mathsf{valid\_c}(x) \lor s_u \ge \mathsf{MAX\_UNCERTAIN}) \land \mathsf{sns} = 1 \land t = 0 \}\\
...
G(e_{30}) &= \{ x \in X \mid \mathsf{valid\_c}(x) \lor (\mathsf{sns} = 0 \land t \ge \mathsf{CAMERA\_FREQ}) \} \\
\end{aligned}
\]
